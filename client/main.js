    ...
    if (isInsideTelegram()) {
      try {
        ...
        if (verified) {
          ...
          logInfo('⚠️ Telegram verification failed - continuing in limited mode');
        }
      } catch (error) {
        logError('Telegram verification error:', error);
        logInfo('⚠️ Continuing without Telegram verification');
      }
    } else {
